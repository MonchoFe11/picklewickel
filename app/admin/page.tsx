...
export default function AdminDashboard() {
  const { matches, deleteMatch, deleteMultipleMatches, duplicateMatch } = useMatches();
  ...
  const filteredMatches = useMemo(() => {
    ...
    // Search filter - updated for new data structure
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      filtered = filtered.filter(match => 
        (match.team1?.players || (match as any).playersTeam1 || []).some((player: any) => 
          (typeof player === 'string' ? player : player.name).toLowerCase().includes(query)) ||
        (match.team2?.players || (match as any).playersTeam2 || []).some((player: any) => 
          (typeof player === 'string' ? player : player.name).toLowerCase().includes(query)) ||
  ...
  }, [matches, searchQuery, statusFilter, sortColumn, sortDirection]);
        ...
        <div className="bg-surface-light rounded-lg p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <input
                type="text"
                placeholder="Search players, tournament, or court..."
                value={searchQuery}
                onChange={(e) => {
