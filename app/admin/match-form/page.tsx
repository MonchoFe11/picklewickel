...
export default function MatchForm() {
  ...
  // Memoized tournaments list
  const tournaments = useMemo(() => getUniqueTournaments(), [getUniqueTournaments]);
  ...
  const [formData, setFormData] = useState({
    date: '',
    time: '',
    status: 'Upcoming' as 'Live' | 'Upcoming' | 'Completed' | 'Forfeit' | 'Walkover',
    tournamentName: '',
    drawName: '',
    round: '',
    court: '',
    ...
    <div className="min-h-screen bg-surface text-onSurface p-4 pb-24">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-surface-light rounded-lg p-6 space-y-6 border border-divider">
          ...
          {/* Set Scores - Hide for Forfeit/Walkover */}
          {!isForfeitOrWalkover && (
            <div>
                ...
                {formData.setScoresTeam1.map((_, index) => (
                  <div key={index} className="flex items-center gap-4">
                    ...
                    <input
                      ...
                      className="w-20 px-3 py-2 bg-surface rounded border border-divider focus:border-green-500 focus:outline-none text-center text-onSurface"
                      data-testid={`set${index + 1}-team1-score`}
                    />
                    <span className="text-onSurface/60">-</span>
                    <input
                      type="number"
                      min="0"
