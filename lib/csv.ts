  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        // Parse team players (NEW FORMAT - individual columns)
        const team1Players = [
        ...
        const isDuplicate = existingMatches.some(existingMatch => {
          // Get first team1 player name (handle both old and new data structures)
          const existingTeam1FirstPlayer = existingMatch.team1?.players?.[0]?.name || 
                                          (existingMatch as any).playersTeam1?.[0] || '';
          
          return existingMatch.date === match.date &&
                 existingMatch.tournamentName === match.tournamentName &&
                 existingTeam1FirstPlayer === team1Players[0].name;
        });
