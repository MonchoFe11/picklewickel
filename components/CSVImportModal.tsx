...
export default function CSVImportModal({ isOpen, onClose }: CSVImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Toast notification with detailed feedback
      const toastMessage = `Import complete: ${importedCount} matches added, ${previewData.errors.length} rows with errors skipped, ${previewData.duplicateCount} duplicates skipped.`;
  ...
  const copyHeaders = () => {
    // NEW FORMAT - Individual player columns with scores and winner
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-surface-light rounded-lg p-4 sm:p-6 w-full max-w-5xl mx-2 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        ...
        {/* CSV Format Help - Updated for new format */}
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full px-3 py-2 bg-surface rounded border border-divider focus:border-green-500 focus:outline-none text-sm"
          />
        </div>
